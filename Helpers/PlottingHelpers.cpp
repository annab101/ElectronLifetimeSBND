...
namespace calib {
    ...
    void plotBand(std::string MPVfileName, std::vector<double> lifetimes, std::vector<int> colours, std::string plotSetup, std::string histName){
        
        TFile f((MPVfileName).c_str());
        std::pair<std::string, std::pair<double,double>> thisSetup = whichSetup[plotSetup];
        TF1 *expoFit = (TF1*)f.Get(("MPVplot_" + thisSetup.first + "_" + histName).c_str());
